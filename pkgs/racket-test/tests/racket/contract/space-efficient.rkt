...
(parameterize ([current-contract-namespace
  ...
  (contract-eval
   '(define guarded
      (contract pos->pos (lambda (x) (* x -2)) 'positive 'negative)))
  (contract-eval
   '(define f1 (contract pos->pos->pos
                         (lambda (f)
                         ...
                         'pos 'neg)))
  (contract-eval
   '(define f2 (contract pos->pos->pos
                         (lambda (f)
                           (unless (has-contract? f)
  ...
  (contract-eval
   '(define guarded-twice (contract pos->pos guarded 'positive2 'negative2)))
  ...
  (test-true
   ...
   '(can-combine? ic c1)) ; can collapse impersonators, the inner chaperone is not chaperone*
  (test-false
   'space-efficient-chap+imp2
   '(can-combine? ic c2))
  (test-true
   'space-efficient-chap+imp3
   '(can-combine? iic c1)) ; see above
  (test-false
  ...
  (contract-eval '(define ci (contract (-> (-> c2 c2) (-> c2 c2))
                                       (contract (-> (-> c1 c1) (-> c1 c1))
                                                 ...
                                                 'inner-pos 'inner-neg)
                                       'pos 'neg)))
  (contract-eval '(define ici (contract (-> (-> c1 c1) (-> c1 c1))
                                        ci
                                        'outer-pos 'outer-neg)))
  (test-false
  ...
  (test-true
   ...
   '(can-combine? ci c2)) ; it's impersonated before the `cc`, but not impersonator*, sook
  (test-true
   'space-efficient-chap+imp12
   '(can-combine? ici c1)) ; ditto
  (test-false
  ...
  (contract-eval
   '(define imp-imp (contract (-> c1 c1)
                              (contract (-> c1 c1) (lambda (x) x) 'pos 'neg)
                              'pos 'neg)))
  (test-true
  ...
  (test/pos-blame
   ...
   '(contract (-> any/c)
              (contract (-> any/c any/c) add1 'inner-pos 'inner-neg)
              'pos 'neg))
  (test/pos-blame
